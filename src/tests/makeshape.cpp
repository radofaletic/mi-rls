...
int main(int argc, char* argv[])
{
  ...
  size_t DC = ( !(sos%2) ) ? (sos-2)/2 : (sos-1)/2;
  for (size_t i=0; i<sos; i++) // NOTE: swap x-y format from Column-major to Row-major for FFTW
    {
      for (size_t j=0; j<sos; j++)
	{
  ...
  for (size_t j=0; j<sos; j++)  // NOTE: swap x-y format from Row-major back to Column-major
    {
      for (size_t i=0; i<sos; i++)
	{
