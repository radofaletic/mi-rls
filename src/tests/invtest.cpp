...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  grid<real> test_grid(gridinputs);
  ...
  SparseMatrix<real> A(0,test_grid.ncells());
  std::valarray<real> b;
  std::valarray<real> cellw;
  ...
  if ( !yesno("read existing matrix",false) )
    {
      ...
      // large objects
      //
      std::valarray<size_t> cellnumbers;
      ...
      size_t meshsize = 32;
      std::valarray<real>* ratt = new std::valarray<real>(test_grid.scale()/10.0f,test_grid.dim());
      std::valarray<real>* ratu = new std::valarray<real>(test_grid.min() + *ratt);
      std::valarray<real>* ratv = new std::valarray<real>(test_grid.max() - *ratt);
      size_t Nrows; size_t Ncols;
      std::vector< line<real> > ray = Tomography::synthetic_rays(rotations, meshsize, *ratu, *ratv, Nrows, Ncols);
      delete ratt;
      delete ratu;
      delete ratv;
      ...
      for (size_t i=0; i<test_grid.ncells(); i++)
	{
	  cellw[i] = test_grid[i];
	}
  ...
  AddDamping(A, b, test_grid, question("matrix damping value",real(0.001)));

  //
  // smoothing
  //
  message("adding neighbour smoothing equations");
  AddSmoothing(A, b, test_grid, question("matrix smoothing value",real(0.1)));
  ...
  // solve Ax=b
  //
  message("inverting "+ctos(typeid(A).name()));
  std::valarray<real> x(real(0), test_grid.ncells());
