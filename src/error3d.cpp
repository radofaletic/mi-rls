...
int main(int argc, char* argv[])
{
	...
	std::vector< std::valarray<real> > zero(4);
	...
	std::valarray<real> difference1 = std::abs(data1-source);
	data1.resize(0);
	real L2error1 = norm(difference1) / norm(source);
	std::valarray<real> difference2(0);
	real L2error2 = real(0);;
	if ( datafile2.size() )
	{
	...
	{
		zero[i] = std::valarray<real>(real(0), zero[i].size());
	}
	...
	message("L2 error for "+datafile1+": "+ntos(L2error1));
	if ( datafile2.size() )
	{
		message("L2 error for "+datafile2+": "+ntos(L2error2));
	}
