...
namespace Tomography
{
  ...
  template<class T> std::vector< line<T> >
    create_rays(const size_t&, const size_t&,                 // Nrows, Ncols
		const size_t&,                                // number of datum
  ...
  template<class T> std::vector< line<T> >
    synthetic_rays(const size_t&,                             // number of angles
		   const size_t&,                             // mesh size
...
/* ---------- projection ---------- */
template<class T> void
Tomography::projection(grid<T>& g,
...
/* ---------- pngwrite ---------- */
/* NOTE: input data should be in
   standard numerical format, ie
   starting at the bottom left.
   This function automatically
   converts this to image format,
   ie starting at the top left.   */
template<class T> bool
Tomography::pngwrite(const std::string& myfilename,
		     ...
		     const bool& tpcy)
{
  ...
  if ( input_data.size() == iNrows * iNcols )
      ...
      for (size_t i=0; i<input_data.size()/(iNrows*iNcols); i++)
	{
      ...
      c_cols = data.size();
      size_t wh_sqr = (c_cols*iNcols+c_cols-1)*(c_cols*iNcols+c_cols-1) + (c_rows*iNrows+c_rows-1)*(c_rows*iNrows+c_rows-1);
      for (size_t i=1; i<data.size()+1; i++)
	{
	  for (size_t j=1; j<data.size()+1; j++)
	      ...
	      if ( i * j < data.size() )
		{
	      ...
	      size_t wh_width = i*iNcols+i-1;
	      size_t wh_height = j*iNrows+j-1;
  ...
  for (size_t d=0; d<data.size(); d++)
    {
      for (size_t j=0; j<iNrows/2; j++)
	{
	  for (size_t i=0; i<iNcols; i++)
	    {
  ...
  if ( IEEEdata )
    {
      size_t nodier = 1600; // number of data in each row
      ...
      for (size_t i=0; i<data.size(); i++)
	{
      ...
      for (size_t i=0; i<IEdata.size(); i+=nodier)
	{
	  size_t dvs = i + nodier;
  ...
  if ( IEEEdata )
    {
      text_ptr[10].key = "ndatarows"; // stores how many data arrays there are
...
/* NOTE: this function converts
   from image format, ie starting
   at the top left, into standard
   numerical format, ie starting
   at the bottom left.           */
template<class T> bool
Tomography::pngread(const std::string& myfilename,
		    ...
		    bool& IEEEdata)
{
  ...
  if ( png_get_text(png_ptr, info_ptr, &text_ptr, &num_text) )
      ...
      if ( IEEEdata )
	{
	  ...
	  for (size_t i=0; i<num_text; i++)
		...
		{
		  size_t data_n = stoi(key.substr(8,key.size()-8));
  ...
  for (size_t d=0; d<output_data.size(); d+=(iNrows*iNcols))
    {
      for (size_t j=0; j<iNrows/2; j++)
	{
	  for (size_t i=0; i<iNcols; i++)
...
/* ---------- create_rays ---------- */
template<class T> std::vector< line<T> >
Tomography::create_rays(const size_t& Nrows, const size_t& Ncols,
			...
			const T& scale)
{
  ...
  size_t count = 0;
  size_t total = angles.size() * ipoints.size();
  debugn("creating "+ntos(total)+" rays: ");
  for (size_t i=0; i<angles.size(); i++)
    {
...
/* ---------- synthetic_rays ---------- */
template<class T> std::vector< line<T> >
Tomography::synthetic_rays(const size_t& nangles,
			   ...
			   size_t& Nrows, size_t& Ncols)
{
  ...
  switch (dim)
    ...
    case 3:
      if ( xdist > ydist )
	{
	  Nrows = size_t( T(meshsize) * ydist / xdist );
	  Ncols = meshsize;
	...
	{
	  Nrows = meshsize;
	  Ncols = size_t( T(meshsize) * xdist / ydist );
  ...
  std::valarray<T> slope = islope;
  size_t count = 0;
  size_t total = nangles * ipoints.size();
  debugn("creating "+ntos(total)+" rays: ");
