...
/// The function "blank_2dgrid" generates a structured 2D mesh in 3D. Note that Nrows is the number of points in the y-direction and Ncols is the number of points in the x-direction
template<class T> std::vector< std::valarray<T> > blank_2dgrid(const unsigned short& = 2,                          // dimension
                                                               const std::size_t& = 128, const std::size_t& = 128, // Nrows, Ncols
                                                               const T& = T(1),                                    // spacing
                                                               const Angle::axes& = Angle::XY);                    // axes

template<class T> std::vector< std::valarray<T> > blank_2dgrid(const std::valarray<T>&,                          // shift vector
                                                               const std::size_t& =128, const std::size_t& =128, // Nrows, Ncols
                                                               const T& = T(1),                                  // spacing
                                                               const Angle::axes& = Angle::XY);                  // axes

template<class T> std::vector< std::valarray<T> > blank_2dgrid(std::size_t& Nrows, std::size_t& Ncols, // Nrows, Ncols
                                                               const std::valarray<T>&,                // lower left point
                                                               const std::valarray<T>&,                // upper right point
                                                               const std::size_t& = 128,               // Nrowscols
                                                               const Angle::axes& = Angle::XY);        // axes

/// each mesh point has data associated with it, and here we save that data for multiple meshes based on the one initial blank_2dgrid mesh
/// b3d file format: number_of_projections, axes, Nrows Ncols, rotation_origin, array_of_initial_3D_points

/// write initial mesh data to the file
template<class T> bool write_b3d_begin(const std::string&,                                // filename
                                       const std::size_t&,                                // number of projections
                                       const Angle::axes&,                                // axes
                                       const std::valarray<T>&,                           // origin of rotation
                                       const std::vector< std::valarray<T> >&,            // points
                                       const std::size_t& Nrows, const std::size_t& Ncols,// Nrows, Ncols
                                       const dataformat& = Unformatted);

/// append projection data to the file
template<class T> bool write_b3d_append(const std::string&,               // filename
                                        const T&,                         // angle
                                        const std::valarray<T>&,          // data
                                        const dataformat& = Unformatted);

/// read projection data from the file
template<class T> bool read_b3d(const std::string&,               // filename
                                Angle::axes&,                     // axes
...
/* ---------- blank_2dgrid ---------- */
/// create a regular lattice with Nrows nodes in the y-direction and Ncols nodes in the x-direction
template<class T> std::vector< std::valarray<T> > blank_2dgrid(const unsigned short& dim,
                                                               ...
                                                               const Angle::axes& axis)
{
    ...
    // grid values are held in Cartesian format, but our 3D-image must be in row-col format
    for (std::size_t i=0; i<std::size_t(std::floor(T(Nrows)/2)); i++)
    {
...
/* ---------- blank_2dgrid ---------- */
/// create a regular lattice with Nrows nodes in the X direction and Ncols nodes in the Y direction and then shift the whole thing in the shifter direction
template<class T> std::vector< std::valarray<T> > blank_2dgrid(const std::valarray<T>& shifter,
                                                               const std::size_t& Nrows,
                                                               const std::size_t& Ncols,
                                                               const T& spacing,
                                                               const Angle::axes& axis)
...
/* ---------- blank_2dgrid ---------- */
/// create a regular lattice with lower left corner at vmin, upper right corner at vmax, and with nn nodes along the longer of the two directions
template<class T> std::vector< std::valarray<T> > blank_2dgrid(std::size_t& Nrows, std::size_t& Ncols,
                                                               ...
                                                               const Angle::axes& axis)
{
    ...
    if ( xdist > ydist )
    {
        Nrows = std::size_t( T(nn) * ydist / xdist );
        Ncols = nn;
    ...
    {
        Nrows = nn;
        Ncols = std::size_t( T(nn) * xdist / ydist );
...
/* ---------- write_b3d_begin ---------- */
/// write the header for a series of 2D meshes in 3D
template<class T> bool write_b3d_begin(const std::string& filename,
                                       const std::size_t& no_proj,
                                       const Angle::axes& axes,
...
/* ---------- write_b3d_append ---------- */
/// append data for a series of 2D meshes in 3D
template<class T> bool write_b3d_append(const std::string& filename,
                                        const T& angle,
                                        const std::valarray<T>& data,
                                        const dataformat& format)
...
/* ---------- read_b3d ---------- */
/// read data for a series of 2D image meshes in 3D
template<class T> bool read_b3d(const std::string& filename,
                                Angle::axes& axes,
...
/* ---------- pngwrite ---------- */
template<class T> bool pngwrite(const std::string& myfilename,
                                ...
                                const std::valarray<T>& my_angles)
{
    ...
    std::size_t wh_sqr = (c_cols*iNcols+c_cols)*(c_cols*iNcols+c_cols) +
    ...
    for (std::size_t i=1; i<data.size()+1; i++)
    {
        for (std::size_t j=1; j<data.size()+1; j++)
        {
            ...
            std::size_t wh_width = i*iNcols+i;
            std::size_t wh_height = j*iNrows+j;
    ...
    if ( tpoints.size() )
    {
        std::size_t sCAL_wcounter = 0;
        std::size_t sCAL_hcounter = 0;
        for (std::size_t i=0; i<tpoints.size(); i++)
        {
            if ( i%Ncols )
            {
    ...
    text_ptr[5].text = (char*)"imglib::pngwrite";
...
/* ---------- pngread ---------- */
template<class T> bool pngread(const std::string& myfilename,
                               std::string& dataname,
                               ...
                               std::valarray<T>& angles)
{
    ...
    if ( png_get_text(png_ptr, info_ptr, &text_ptr, &num_text) )
    {
        ...
        if ( data.size() )
        {
            for (std::size_t i=0; i<num_text; i++)
            {
                ...
                {
                    std::size_t dj = std::stol(key.substr(5,key.size()-5));
    ...
    if ( !IEEEdata )
    {
        ...
        for (std::size_t i=1; i<cidata.size(); i++)
        {
            cidata[i] = (cidata[i-1])+1;
        }
        for (std::size_t i=0; i<Nrows; i++)
        {
            for (std::size_t j=0; j<Ncols*sizeof(png_uint_16); j++)
            {
