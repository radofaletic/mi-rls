...
bilinear_interpolate(std::valarray<T>&, const std::valarray<T>&, const size_t& = 0, const size_t& = 0, const bool& = false);

template<class T> void
nn_interpolate(std::valarray<T>&, const std::valarray<T>&, const size_t& = 0, const size_t& = 0);
...
template<class T> void
bilinear_interpolate(std::valarray<T>& udata, const std::valarray<T>& iangles,
		     const size_t& xres, const size_t& yres, const bool& extend)
{
  ...
  size_t oresolution = udata.size() / iangles.size();
  size_t DC = ( !(oresolution%2) ) ? (oresolution-2)/2 : (oresolution-1)/2;
  size_t resolution = oresolution - DC;
  ...
  size_t iresolution = ( xres != oresolution ) ? xres : oresolution;
  size_t jresolution = ( yres != oresolution ) ? yres : oresolution;
  std::vector< std::valarray<T> > points(iresolution*jresolution, std::valarray<T>(2));
  for (size_t j=0; j<jresolution; j++)
    {
      for (size_t i=0; i<iresolution; i++)
	{
  ...
  for (size_t i=0; i<points.size(); i++)
    {
      for (size_t theta=0; theta<angles.size()-1; theta++)
	{
	  if ( eq(points[i][1], angles[theta]) ) // point lies along one of the radial axes
	    {
	      for (size_t r=0; r<resolution-1; r++)
		{
...
{
  size_t resolution = udata.size() / angles.size();
  ...
  size_t Nrows = ( yres ) ? yres : resolution;
  size_t Ncols = ( xres ) ? xres : resolution;
