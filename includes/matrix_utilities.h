/**
 ...
 5th August 2004
 20th April 2022, updated to C++20
 */
...
template<class T> void AddDamping(SparseMatrix<T>& A, std::valarray<T>& b, const grid<T>& g, const T& damp)
{
    ...
    if ( A.AddDiagonal(g.ncells(),damp*g.scale()) )
    {
        ...
        debug(" - added a " + std::to_string(sor) + "*" + std::to_string(g.ncells()) + " submatrix with diagonal " + std::to_string(damp*g.scale()));
    }
    else
    {
...
template<class T> void AddSmoothing(SparseMatrix<T>& A, std::valarray<T>& b, const grid<T>& g, const T& smoothing)
{
    debug("AddSmoothing", "adding smoothing to matrix system...");
    
    const std::valarray<bool>* refs = A.Referenced();
    
    std::vector< std::valarray<std::size_t> > s_neigh(0);
    std::vector<std::size_t> s_i(0);
    for (std::size_t i=0; i<g.ncells(); i++)
    {
        if ( !((*refs)[i]) )
        {
            continue;
        }
        std::valarray<std::size_t> gn = g.get_neighbours(i);
        std::valarray<bool> gnref(true,gn.size());
        for (std::size_t j=0; j<gn.size(); j++)
        {
            if ( !((*refs)[gn[j]]) )
            {
                gnref[j] = false;
    ...
    if ( A.AddNReverse(s_neigh, s_i, smoothing) )
    {
        ...
        b.resize(bv.size()+sor,T(0));
        b[std::slice(0, bv.size(), 1)] = bv;
        debug(" - added " + std::to_string(sor) + " smoothing rows, with smoothing " + std::to_string(smoothing));
    }
    else
    {
