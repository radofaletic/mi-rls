...
// `segment' class
template<class T>
class segment : public compact_shape<T>
{
 private:
  line<T> container_;
 public:
...
segment<T>::init(const std::valarray<T>* pt1, const std::valarray<T>* pt2)
{
...
segment<T>::segment(const std::valarray<T>* pt1, const std::valarray<T>* pt2)
{
  this->init(pt1, pt2);
}
/* ----------------------------- */
...
/* ---------- distance_p ---------- */
template<class T> T
segment<T>::distance_p(const std::valarray<T>* pt) const
{
...
segment<T>::contains_p(const std::valarray<T>* pt) const
{
  if ( this->dim() != pt->size() )
    {
      throw; return false;
...
/* ---------- intersect ---------- */
template<class T> T
segment<T>::intersect(const line<T>& line2, std::valarray<T>& cut, bool& parallel) const
{
  cut.resize(0);
  parallel = false;
...
segment<T>::face(const unsigned short& en) const
{
  return 0;
}
/* -------------------------- */
