/**
 ...
 18th April 2004
 16th April 2022, updated to C++20
 */
...
template<class T> inline T r2d(const T& radians) { return ( is_zero(radians) ) ? T(0) : radians * T(180) / std::numbers::pi_v<T>; };
...
template<class T> inline T d2r(const T& degrees) { return ( is_zero(degrees) ) ? T(0) : degrees * std::numbers::pi_v<T> / T(180); };
...
template<class T> inline void C2p(std::valarray<T>& point)
{
    T x = point[0];
    T y = point[1];
    point[0] = std::sqrt(x*x+y*y);
    if ( is_zero(x) && is_zero(y) )
    {
        point[1] = T(0);
        
    }
    else if ( is_zero(x) && y < T(0) )
    {
        point[1] = T(270);
        
    }
    else if ( is_zero(x) && T(0) < y )
    {
        point[1] = T(90);
        
    }
    else if ( x < T(0) && is_zero(y) )
    {
        point[1] = T(180);
        
    }
    else if ( T(0) < x && is_zero(y) )
    {
        point[1] = T(0);
    ...
    {
        point[1] = std::atan2(std::abs(y), std::abs(x));
        point[1] = Angle::r2d(point[1]);
        if ( x < T(0) && y < T(0) )
        {
            point[1] += T(180);
        }
        ...
        {
            point[1] = T(180) - point[1];
            
        }
        else if ( y < T(0) )
        {
            point[1] = T(360) - point[1];
