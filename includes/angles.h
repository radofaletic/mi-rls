...
namespace Angle
{
  ...
  const long double
    pi = (long double)(4) * std::atan((long double)(1));

  // convert radians to degrees
  template<class T> inline T
    r2d(const T& radians)
    { return ( is_zero(radians) ) ? T(0) : radians * T( (long double)(180) / Angle::pi ); };

  // convert degrees to radians
  template<class T> inline T
    d2r(const T& degrees)
    { return ( is_zero(degrees) ) ? T(0) : degrees * T( Angle::pi / (long double)(180) ); };

  // convert Cartesian to polar coordinates (x,y) to (r,theta)
  template<class T> inline void
    C2p(std::valarray<T>& point)
    {
