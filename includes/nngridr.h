...
public:
    ...
    NNG_Matrix(const std::size_t& nrows, const std::size_t& ncols, const T& value = T(0)) { init(nrows, ncols, value); };
    ~NNG_Matrix() { free(); };
    void init(const std::size_t& nrows, const std::size_t& ncols, const T& value = T(0))
    {
    ...
    T operator()(const std::size_t& row, const std::size_t& col) const { return (*Mdata_)[row * Mcols_ + col]; };
    T& operator()(const std::size_t& row, const std::size_t& col) { return (*Mdata_)[row * Mcols_ + col]; };
};
...
template<class T> void nngridrc(const std::vector< std::valarray<T> >&, std::valarray<T>&, const std::size_t&, const std::size_t&, const bool& = false);
template<class T> void nngridr(std::vector< std::valarray<T> >&, std::valarray<T>&, const std::size_t&, const std::size_t&, const bool& = false);
...
template<class T> void nngridrc(const std::vector< std::valarray<T> >& ucpoints,
...
template<class T> void nngridr(std::vector< std::valarray<T> >& upoints,
                               std::valarray<T>& udata,
                               const std::size_t& Nrows,
                               const std::size_t& Ncols,
                               const bool& verbose)
{
    ...
    {
        debug("nngridr", "number of points = " + std::to_string(upoints.size()) + ", but number of datum = " + std::to_string(udata.size()));
        throw;
        return;
    }
    nngridr_ini<T> ini;
    ...
    for (std::size_t i=0; i<upoints.size(); i++)
    {
    ...
    if ( ReadData(nng, ini, upoints, udata, verbose) )
    {
        if ( verbose )
        {
            debug(std::to_string(nng.datcnt) + " data were input");
        }
        if ( ini.ichoro )
        {
            ChoroPleth(nng, ini);
...
template<class T> void nngridr_ini<T>::ShowState(const bool& atime, const bool& verbose)
{
    ...
    if ( verbose )
    {
        ...
        if ( igrad )
        {
            ...
            debug("Tautness parameter #1 " + std::to_string(bI));
            debug("Tautness parameter #2 " + std::to_string(bJ));
            if ( non_neg )
            {
        ...
        else
        {
            ...
            debug("Null value is " + std::to_string(nuldat));
        }
        debug("................... Gridded Region Parameters ....................");
        debug("Nodes along x: " + std::to_string(x_nodes));
        debug("Nodes along y: " + std::to_string(y_nodes));
        
        debug("Least x: " + std::to_string(xstart));
        debug("Least y: " + std::to_string(ystart));
        
        debug("x spacing: " + std::to_string(xspace));
        debug("y spacing: " + std::to_string(yspace));
        
        debug("Greatest x: " + std::to_string(xterm));
        debug("Greatest y: " + std::to_string(yterm));
