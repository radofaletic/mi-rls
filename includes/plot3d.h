...
/*
 NOTES
 - see `Plot3D.txt' or `Plot3D.html' for more information
 about this data/file format
 */
...
template<class T> bool write_neighbours(const std::vector< std::valarray<T> >&,
										...
										const std::valarray<size_t>&,
										const std::valarray<size_t>&,
										const std::vector< std::valarray<size_t> >&,
										const std::string&,
										const dataformat& = Unformatted);
...
Plot3D::read_interface(std::vector< std::valarray<T> >& nodes,
					   ...
					   bool& multidomain)
{
	...
	for (size_t loop=0; loop<nodes.size(); loop++)
	{
		nodes[loop][0] = X[loop];
		nodes[loop][1] = Y[loop];
		nodes[loop][2] = Z[loop];
			 ...
			 const bool& blanking)
{
	...
	Nz.resize(domains);
	size_t numnodes = 0;
	for (size_t loop=0; loop<domains; loop++)
	{
	...
	for (size_t loop=0; loop<domains; loop++)
	{
		Fortran::read_byte_info(gridfile, isize, byte_swapping, format);
		// I values
		for (size_t k=0; k<Nz[loop]; k++)
		{
			for (size_t j=0; j<Ny[loop]; j++)
			{
				for (size_t i=0; i<Nx[loop]; i++)
				{
...
Plot3D::read_data_interface(std::valarray<T>& data,
							std::string& dataname,
							std::valarray<size_t>& Nx,
							std::valarray<size_t>& Ny,
							std::valarray<size_t>& Nz,
							...
							bool& multidomain)
{
	...
	// see if user has already opened the grid file,
	// and if so try to open the corresponding Q-file
	...
	else
	{
		...
		if ( ft > 0 )
		{
			...
			message("will read variable "+ntos(variable)+" ("+vars[variable-1]+") from the Q-file `"+qfilename+"'");
			if ( variable > 5 )
			{
				variable -= 5;
			}
...
/* reads Plot3D data (or Q-file)   */
template<class T> bool
Plot3D::read_data(std::valarray<T>& data1,
...
/* reads Plot3D data (or Q-file)   */
template<class T> bool
Plot3D::extract_data(std::valarray<T>& data,
					 const std::string& qfilename,
					 const dataformat& format,
					 const dataprecision& fsize,
					 const bool& multidomain,
...
/* ---------- write_interface ---------- */
template<class T> bool
Plot3D::write_interface(const std::vector< std::valarray<T> >& nodes,
						const std::valarray<size_t>& Nx,
						const std::valarray<size_t>& Ny,
						const std::valarray<size_t>& Nz,
						const std::string& gfilename)
{
	...
	// sort out dimensions
	size_t numnodes = 0;
	for (size_t loop=0; loop<Nx.size(); loop++)
	{
		numnodes += Nx[loop] * Ny[loop] * Nz[loop];
	}
...
/* ---------- write ---------- */
template<class T> bool
Plot3D::write(const std::valarray<T>& X,
			  ...
			  const dataformat& format)
{
	std::valarray<size_t> vNx(Nx, 1);
	std::valarray<size_t> vNy(Ny, 1);
	std::valarray<size_t> vNz(Nz, 1);
	Plot3D::write(X, Y, Z, B, vNx, vNy, vNz, filename, format, Single, false, false);
}
/* --------------------------- */
...
template<class T> bool
Plot3D::write_data_interface(const std::valarray<T>& data,
...
/* ---------- write_data ---------- */
template<class T> bool
Plot3D::write_data(const std::valarray<T>& data1,
				   ...
				   const dataformat& format)
{
	...
	std::valarray<size_t> vNy(Ny, 1);
	std::valarray<size_t> vNz(Nz, 1);
	Plot3D::write_data(data1, data2, data3, data4, data5, vNx, vNy, vNz, qfilename, format, Single, false);
}
/* -------------------------------- */
...
/* ---------- write_var ---------- */
bool
Plot3D::write_var(const std::string& vfilename,
...
/* ---------- get_vertices ---------- */
/* finds the vertices for the given
 Plot3D cell                        */
void
Plot3D::get_vertices(const unsigned short& dim,
					 const std::valarray<size_t>& Nx,
					 const std::valarray<size_t>& Ny,
					 const std::valarray<size_t>& Nz,
					 std::valarray<size_t>& vertices,
					 const size_t& cell)
{
	size_t point_zero = 0;
	// find which domain the cell lies in
	size_t domain = Nx.size();
	size_t zero = 0;
	size_t nnx, nny, nnz;
	for (size_t i=0; i<Nx.size(); i++)
	{
...
/* ---------- get_surrounds ---------- */
/* finds the surrounds cells for a given
 node                                */
void
Plot3D::get_surrounds(const unsigned short& dim,
					  const std::valarray<size_t>& Nx,
					  const std::valarray<size_t>& Ny,
					  const std::valarray<size_t>& Nz,
					  std::valarray<size_t>& surrounds,
					  const size_t& node)
{
	surrounds.resize(0);
	std::vector<size_t> surr;
...
/* ---------- n2c ---------- */
template<class T> void
Plot3D::n2c(const std::vector< std::valarray<T> >& nodes,
			const std::valarray<size_t>& Nx,
			const std::valarray<size_t>& Ny,
			const std::valarray<size_t>& Nz,
...
/* ---------- c2n ---------- */
template<class T> void
Plot3D::c2n(const std::vector< std::valarray<T> >& nodes,
			const std::valarray<size_t>& Nx,
			const std::valarray<size_t>& Ny,
			const std::valarray<size_t>& Nz,
...
/* ---------- write_neighbours ---------- */
template<class T> bool
Plot3D::write_neighbours(const std::vector< std::valarray<T> >& nodes,
						 const std::valarray<size_t>& Nx,
						 const std::valarray<size_t>& Ny,
						 const std::valarray<size_t>& Nz,
						 const std::vector< std::valarray<size_t> >& neighbours,
						 const std::string& filename,
						 const dataformat& format)
{
...
/* ---------- to_PNG ---------- */
template<class T> bool
Plot3D::to_PNG(const std::valarray<T>& X,
			   const std::valarray<T>& Y,
			   const std::valarray<T>& Z,
			   const std::valarray<size_t>& Nx,
			   const std::valarray<size_t>& Ny,
			   ...
			   const Plot3D::values& get_switch)
{
	...
	switch(get_switch)
	{
		case Plot3D::NODES:
			...
			{ // TWO_zx
				Ncols = Nz[0];
				Nrows = Nx[0];
				for (size_t j=0; j<Nx[0]; j++)
				{
					for (size_t i=0; i<Nz[0]; i++)
					{
		...
		case Plot3D::CENTROIDS:
			...
			{ // TWO
				...
				for (size_t j=0; j<Nrows; j++)
				{
					for (size_t i=0; i<Ncols; i++)
					{
						size_t i1 = j*(Ncols+1)+i;
						size_t i2 = j*(Ncols+1)+i+1;
						size_t i3 = (j+1)*(Ncols+1)+i;
						size_t i4 = (j+1)*(Ncols+1)+i+1;
						std::valarray<T> p1(3);
						std::valarray<T> p2(3);
						std::valarray<T> p3(3);
						std::valarray<T> p4(3);
	...
	for (size_t i=0; i<size_t(std::floor(double(Nrows)/2)); i++)
	{
	...
	png_set_pCAL(png_ptr, info_ptr, "Plot3D", X0, X1, PNG_EQUATION_LINEAR, nparams, units, params);
	...
	std::string sdn = "";
	size_t nodier = 2000; // number of data in each row
	for (size_t i=0; i<ddata.size(); i+=nodier)
	{
		size_t dvs = i + nodier;
	...
	for (size_t i=0; i<IEEEsd.size(); i++)
	{
		IEEEsd[i] = "IEEEdata" + ntos(i);
	}
	sdn = ntos(IEEEs.size());
	...
	text_ptr[2].text = "slice of a Plot3D grid";
