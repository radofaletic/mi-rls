/*
 ...
 A collection of C++ routines for extra arithmetic
 functionionality.
 ...
 - norm
 - normalise
 There are also some useful template classes defining
 some scalar arithmetic functionals.
...
template<class T> T
dot(const std::valarray<T>&, const std::valarray<T>&);

// float vec_norm_squared = norm2(vec);
template<class T> T
norm2(const std::valarray<T>&);

// float vec_norm = norm(vec);
template<class T> T
norm(const std::valarray<T>&);

// float norm_of_vec2-vec1 = norm(vec1, vec2);
template<class T> T
norm(const std::valarray<T>*, const std::valarray<T>*);

// float vec_norm = normalise(vec);
template<class T> T
normalise(std::valarray<T>&);

// std::valarray<float> vec_cross = cross(vec1,vec2);
template<class T> std::valarray<T>
cross(const std::valarray<T>&, const std::valarray<T>&);

// std::valarray<float> vec_cross_of_differences = cross(vec1,vec2,vec3);
template<class T> std::valarray<T>
cross(const std::valarray<T>*, const std::valarray<T>*, const std::valarray<T>*);

// bool are_equal_within_limits = eq(a, b);
template<class T> bool
eq(const T&, const T& = T(0), const T& = T(1));

// bool are_same = v_eq(v1, v2);
template<class T> bool
v_eq(const std::valarray<T>&, const std::valarray<T>&, const T& = T(1));
...
template<class T> bool
v_eq(const std::valarray<T>&, const T& = T(0), const T& = T(1));

// bool are_same_or_negatives = v_pm_eq(v1, v2);
template<class T> bool
v_pm_eq(const std::valarray<T>*, const std::valarray<T>*, const T& = T(1));

// bool is_zero = is_zero(a);
template<class T> bool
is_zero(const T&, const T& = T(1));

// bool is_zero = is_zero(v);
template<class T> bool
is_zero(const std::valarray<T>&, const T& = T(1));

// void vertical_flip(myvec, size_of_row);
template<class T> void
vertical_flip(std::valarray<T>&, const size_t& = 1);

// void horizontal_flip(myvec, size_of_col);
template<class T> void
horizontal_flip(std::valarray<T>&, const size_t& = 1);

// remove_duplicates(vec);
template<class T> void
remove_duplicates(std::vector<T>&);

// remove_duplicates(vec_of_vecs);
template<class T> void
remove_duplicates(std::vector< std::valarray<T> >&);

// float distance = maximise_distance(vec_of_vecs);
template<class T> T
maximise_distance(const std::vector< std::valarray<T> >&);

// rotate_matrix(Nrows, Ncols, image);
template<class T> void
rotate_matrix(size_t&, size_t&, std::valarray<T>&, const bool& = true);

// int power = ipow(init_int, ipower);
template<class T> T
ipow(const T&, const size_t&);
...
public:
	...
	two_numbers(const size_t& nint, const T& nre) : n_integer(nint), n_real(nre) { };
	~two_numbers() { };
};
...
dot(const std::valarray<T>& u, const std::valarray<T>& v)
{
	T dp = (u*v).sum();
...
template<class T> inline T
norm(const std::valarray<T>* v1, const std::valarray<T>* v2)
{
...
cross(const std::valarray<T>& v1, const std::valarray<T>& v2)
{
...
template<class T> inline std::valarray<T>
cross(const std::valarray<T>* v1, const std::valarray<T>* v2, const std::valarray<T>* v3)
{
	std::valarray<T> x(T(0), v1->size());
...
/* ---------- eq ---------- */
/* determins if a and b are
 equal, allowing for
 numerical innacuracies   */
template<class T> inline bool
eq(const T& a, const T& b, const T& e)
{
...
/* ---------- v_eq ---------- */
template<class T> inline bool
v_eq(const std::valarray<T>& u, const std::valarray<T>& v, const T& e)
{
...
/* ---------- v_eq ---------- */
template<class T> inline bool
v_eq(const std::valarray<T>& v, const T& n, const T& e)
{
	if ( !v.size() )
	{
		return false;
...
/* ---------- v_pm_eq ---------- */
template<class T> inline bool
v_pm_eq(const std::valarray<T>* u, const std::valarray<T>* v, const T& e)
{
	if ( !v->size() || u->size() != v->size() )
	{
		return false;
...
/* ---------- is_zero ---------- */
/* determines if a and b and equal,
 allowing for numerical innacuracies   */
template<class T> inline bool
is_zero(const T& a, const T& e)
{
	return eq(a, T(0), e);
}
/* ----------------------------- */
...
/* ---------- is_zero ---------- */
template<class T> inline bool
is_zero(const std::valarray<T>& v, const T& e)
{
	return v_eq(v, T(0), e);
}
/* ---------------------------- */
...
vertical_flip(std::valarray<T>& v, const size_t& row_size)
{
	if ( v.size()%row_size )
	{
		throw; return;
	}
	for (size_t i=0; i<(v.size()/row_size)/2; i++)
	{
...
/* ---------- horizontal_flip ---------- */
template<class T> inline void
horizontal_flip(std::valarray<T>& v, const size_t& col_size)
{
	if ( v.size()%col_size )
	{
		throw; return;
	}
	for (size_t j=0; j<v.size()/col_size; j++)
	{
...
/* ---------- remove_duplicates ---------- */
template<class T> void
remove_duplicates(std::vector<T>& v)
{
...
/* ---------- remove_duplicates ---------- */
template<class T> void
remove_duplicates(std::vector< std::valarray<T> >& v)
{
	bool erc = false;
	while ( !erc && v.size() > 1 )
	{
...
/* ---------- maximise_distance ---------- */
template<class T> T
maximise_distance(const std::vector< std::valarray<T> >& v)
{
	T distance = T(0);
	for (size_t i=0; i<v.size(); i++)
	{
...
/* ---------- rotate_matrix ---------- */
/* rotate an image 90 degrees in the
 clockwise direction, or
 anti-clockwise if dir is false      */
template<class T> void
rotate_matrix(size_t& Nrows, size_t& Ncols, std::valarray<T>& m, const bool& dir)
{
...
/* ---------- ipow ---------- */
template<class T> T
ipow(const T& base, const size_t& power)
{
	T ret = T(1);
